...
function toLegacy(raw) {
  ...
  const cfg = {
    ...
    // The native asset, distinct from the pricing numeraire. Carried only when
    // fully specified; anything less leaves it unverified rather than assumed.
    ...
    // The widest block span this chain's RPC will answer a getLogs for. Arc refuses
    // anything over ~1,000 and does not say so in the error, so a scan that assumes
    // a generous range fails outright instead of reading anything.
    ...(Number(chain.maxLogRange) > 0 ? { maxLogRange: Number(chain.maxLogRange) } : {}),
