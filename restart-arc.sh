#!/usr/bin/env bash
# Restart the Arc dashboard on 8797, one instance only.
#
...
LOG="${LOG:-$HERE/arc-dashboard.log}"
...
cd "$HERE"
nohup node server.js --data-dir="$DATA" --port="$PORT" --no-loops --no-services >> "$LOG" 2>&1 &
started=$!
...
echo "started pid $(pids) on :$PORT  (data $DATA, log $LOG)"
